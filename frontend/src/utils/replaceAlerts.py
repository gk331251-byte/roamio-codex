...
ALERT_PATTERNS = [
    ...
    # Validation alerts
    (r'alert\(["\']Select valid locations["\']?\)', 'validationError("Please select valid locations")'),
    (r'alert\(["\'](?:Login required)["\']?\)', 'authError()'),
    
    # Success alerts
    (r'alert\(["\'](?:Report submitted|Draft saved!)["\']?\)', 'success("$1")'),
    (r'alert\(["\']Caption copied to clipboard["\']?\)', 'success("Caption copied to clipboard")'),
    (r'alert\(["\']You\'ve left the group\.["\']?\)', 'success("You\'ve left the group")'),
    
    # Error alerts
    (r'alert\(["\'](?:Failed to.*|Error.*)["\']?\)', 'warning("$1")'),
    
    # General info alerts
    (r'alert\(["\']Time limit too short.*["\']?\)', 'warning("Time limit too short for full quest; using first stop only")'),
]

CONFIRM_PATTERNS = [
    # Delete confirmations
    (r'window\.confirm\(["\']Delete this quest\?["\']?\)', 'await confirmDelete("this quest")'),
    (r'window\.confirm\(["\']Delete account permanently\?["\']?\)', 'await confirmDelete("your account")'),
    
    # Share confirmations
    (r'window\.confirm\(["\']Share this quest publicly\?["\']?\)', 'await confirm("Share this quest publicly?")'),
]
