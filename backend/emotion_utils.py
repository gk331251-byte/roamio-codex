...
def filter_and_score_places(
    places: List[Dict[str, Any]], selected_moods: List[str], max_results: int = 5
) -> List[Dict[str, Any]]:
    """
    ...
    for mood in original_moods:
        ...
        if mood_lower in MOOD_FALLBACKS:
            for fallback in MOOD_FALLBACKS[mood_lower]:
                fallback_combo = [fallback] + [
                    m for m in original_moods if m.lower() != mood_lower
                ]
...
def detect_location_type(
    places_results: List[Dict[str, Any]], population_density: str = None
) -> str:
    """
...
def determine_quest_type(
    place_count: int, time_limit: int, moods: List[str]
) -> Dict[str, Any]:
    """
...
def calculate_adaptive_radius(
    base_radius: int, location_type: str, attempt: int, time_limit: int
) -> int:
    """
    Calculate intelligent radius based on location type, attempt number, and time constraints.
    """
    # Get location intelligence
    location_info = LOCATION_INTELLIGENCE.get(
        location_type, LOCATION_INTELLIGENCE["suburban"]
    )
...
def generate_smart_fallback_message(
    original_moods: List[str], fallback_moods: List[str], radius: int, quest_type: str
) -> str:
    """
    ...
    if fallback_moods != original_moods:
        ...
        messages.append(
            f"We adapted your {original_str} mood to {fallback_str} for better local options"
        )

    if radius > 2000:
        radius_km = radius / 1000
        messages.append(
            f"Expanded search area to {radius_km:.0f}km to find great places"
        )

    if quest_type != "Standard Quest":
        messages.append(
            f"Created a {quest_type} perfectly suited to available locations"
        )
...
def generate_filtered_quest_payload(
    all_places: List[Dict[str, Any]], moods: List[str], time_limit: int
) -> Dict[str, Any]:
    """
