...
class OpenAICircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self.failure_count >= self.failure_threshold:
            self.state = "OPEN"
            logger.warning(f"OpenAI Circuit breaker OPENED after {self.failure_count} failures")
...
class OpenAISafetyManager:
    ...
    async def generate_quest_narrative(
        ...
        try:
            ...
            safe_places = [self._sanitize_input(place) for place in place_names[:5]]  # Limit places
            
            prompt = self._create_safe_prompt(safe_places, safe_moods)
            
            logger.info(f"[{request_id}] Generating quest narrative with OpenAI (safe mode)")
            ...
            if not filter_result.is_safe:
                logger.warning(f"[{request_id}] Content filter flagged issues: {filter_result.issues_found}")
                self._record_content_filter_issue(filter_result.issues_found)
                
                # Use filtered content if issues are minor, fallback if serious
                if any(issue in ['violence', 'inappropriate', 'hate'] for issue in filter_result.issues_found):
                    return self._get_fallback_text(place_names, moods), "template_content_filtered"
            ...
            logger.info(f"[{request_id}] Successfully generated safe quest narrative ({tokens_used} tokens)")
